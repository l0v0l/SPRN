...
def train(args):
    ...
    for _ in tqdm(range(args.epoch)):
        for _, (image, mask, sp) in enumerate(train_loader):
            ...
            for i in range(10):
                output=model(image)
                loss=seg_criterion(sp[:,i,:,:], output[:,0,:,:])
                loss.backward()
                optimizer.step()
            ...
            if train_iter % args.val_loss_every_iters == 0: 
                ...
                with torch.no_grad():
                    for _, (val_image, val_mask, val_sp) in enumerate(val_loader):
                        ...
                        loss = seg_criterion(val_sp, val_output)
                    ...
                    for _, (test_image, test_mask, test_sp) in enumerate(test_loader):
                        ...
                        loss = seg_criterion(test_sp, test_output)
...
if __name__=='__main__':
    parser = argparse.ArgumentParser(description='RNSP stage 1 training')
    parser.add_argument('--lr', help='learning rate', default=1e-3)
    parser.add_argument('--in_folder', help='dataset folder', default='POM_train_SPIE_sp_distance_interp')
    parser.add_argument('--batch_size', help='batch size', default=40)
    parser.add_argument('--epoch', help='epochs', default=50)
